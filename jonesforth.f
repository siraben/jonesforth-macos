\ -*- text -*-
\	A sometimes minimal FORTH compiler and tutorial for macOS / AArch64 systems. -*- asm -*-
...
\ Cell size constants for 64-bit.
: CELL 8 ;
: CELL+ 8+ ;
: CELL- 8- ;
...
: PICK ( x_u ... x_1 x_0 u -- x_u ... x_1 x_0 x_u )
	...
	CELL *		( multiply by the word size )
...
: S" IMMEDIATE		( -- addr len )
	STATE @ IF	( compiling? )
		...
		CELL-		( subtract cell (because we measured from the start of the length word) )
...
: SEE
	...
	WHILE
		...
		' LITSTRING OF		( is it LITSTRING ? )
			...
			SWAP CELL+ SWAP		( end start+cell length )
			2DUP TELL		( print the string )
			'"' EMIT SPACE		( finish the string with a final quote )
			+ ALIGNED		( end start+cell+len, aligned )
			CELL-			( because we're about to add cell below )
		ENDOF
...
: CATCH		( xt -- exn? )
	DSP@ CELL+ >R		( save parameter stack pointer (+cell because of xt) on the return stack )
...
: ARGV ( n -- str u )
	1+ CELLS ARGS @ +	( get the address of argv[n] entry )
...
(
	...
	NOTE: On macOS, environment is not stored after argv.  This word is a stub.
)
: ENVIRON	( -- addr )
...
(
	...
	On macOS we use a fixed 1MB data segment, so UNUSED just computes how much
	is left from HERE to the end of that region.  We store the end address
	as a constant computed at startup.
)
: UNUSED	( -- n )
	1048576		( size of data_segment in bytes )
	HERE @		( current position )
	LATEST @	( we need base of data segment -- approximate via HERE initial )
	-		( bytes used )
	-		( bytes remaining )
...
( Equivalent to the NEXT macro on AArch64: ldr x8,[x26],#8 / ldr x9,[x8] / br x9 )
: NEXT IMMEDIATE 48 C, 87 C, 40 C, F8 C, 09 C, 01 C, 40 C, F9 C, 20 C, 01 C, 1F C, D6 C, ;
...
HEX
( Check for AArch64 NEXT: F8408748 F9400109 D61F0120 - 12 bytes )
: =NEXT		( addr -- next? )
	   DUP C@ 48 <> IF DROP FALSE EXIT THEN
	1+ DUP C@ 87 <> IF DROP FALSE EXIT THEN
	1+ DUP C@ 40 <> IF DROP FALSE EXIT THEN
	1+ DUP C@ F8 <> IF DROP FALSE EXIT THEN
	1+ DUP C@ 09 <> IF DROP FALSE EXIT THEN
	1+ DUP C@ 01 <> IF DROP FALSE EXIT THEN
	1+ DUP C@ 40 <> IF DROP FALSE EXIT THEN
	1+ DUP C@ F9 <> IF DROP FALSE EXIT THEN
	1+ DUP C@ 20 <> IF DROP FALSE EXIT THEN
	1+ DUP C@ 01 <> IF DROP FALSE EXIT THEN
	1+ DUP C@ 1F <> IF DROP FALSE EXIT THEN
	1+     C@ D6 <> IF      FALSE EXIT THEN
	TRUE
;
DECIMAL
